...
class FeatureProcessor():
    ...
    def get_surface_feature_for_residue(self, res_id):  # 为每个残基生成表面特征，并返回其平均值
        ...
        # 计算这些顶点的法向量
        normal_vectors = self.normalv[vertex_indices]
        ...
        avg_normal_vector = torch.mean(torch.tensor(normal_vectors), dim=0)

        # 获取 BOARD 特征
        avg_BOARD = torch.mean(self.BOARD_graph[vertex_indices])
        ...
        surface_feature = torch.cat([torch.tensor([avg_mean_curvature, avg_gaussian_curvature]),
                                     avg_normal_vector,
                                     torch.tensor([shape_index, curvedness, avg_BOARD])], dim=0)
    ...
    def save_residue_features(self, all_features, output_dir):  # 保存所有残基特征到pkl文件
        output_file = os.path.join(output_dir, "all_protein_features.pkl")
...
if __name__ == '__main__':
    dir_opts = {
        'protein_list': '.Data/pdbid.txt',
        'data_output': './data_process/surface/protein/out_put',
        'pdb_files': '.Data/pdb_files/protein_pdb',
        'ply_files': './data_process/surface/protein/ply',
        'xyzrn_files': './data_process/surface/protein/xyzrn'
    }

    proteins = load_proteins(dir_opts['protein_list'])
    all_features = {}
